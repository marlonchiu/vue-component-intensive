<template>
  <div class="table-render2">
    <h2 style="color: red;">实战 6：可用 slot-scope 自定义列的表格组件 —— Table（方案 2）</h2>
    <hr></hr>
      ...
      <template slot-scope="{ row, index }" slot="name">
        ...
        <template slot-scope="{ row, index }" slot="action">
          <div v-if="editIndex === index">
            ...
            <button @click="editIndex = -1" style="margin-left: 6px;">取消</button>
          </div>
          <div v-else>
...
<script>
  import TableRender from '_com/table-render/table.vue'
  export default {
    data () {
      return {
        columns: [
